...
describe('doctor', () => {
  ...
  describe('generateReport()', () => {
    it('should return a formatted string report', () => {
      const checks = [
        { name: 'Node.js 版本', status: 'ok', message: 'v20 ok' },
        { name: '项目依赖', status: 'ok', message: 'installed' },
      ];
    ...
    it('should count errors and warnings correctly', () => {
      const checks = [
        { name: 'A', status: 'error', message: 'bad', fix: 'do something' },
        { name: 'B', status: 'warn', message: 'meh', fix: 'do else' },
        { name: 'C', status: 'ok', message: 'good' },
      ];
      const report = generateReport(checks);
  ...
  describe('generateJsonReport()', () => {
    it('should return a JSON report object', () => {
      const checks = [
        { name: 'Node.js 版本', status: 'ok', message: 'v20 ok' },
        { name: '项目依赖', status: 'ok', message: 'installed' },
        { name: 'WebBridge', status: 'error', message: 'down', fix: 'start' },
      ];
