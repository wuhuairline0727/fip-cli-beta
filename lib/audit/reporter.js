...
function generateTextReport(result) {
  ...
  if (result.fields.buyer_name) lines.push(`购买方: ${result.fields.buyer_name}`);
  if (result.fields.seller_name) lines.push(`销售方: ${result.fields.seller_name}`);
  lines.push('');

  // 金额信息
  lines.push('【金额信息】');
  if (result.fields.contract_amount !== undefined) {
    lines.push(`合同总金额: ${formatFieldAmount(result.fields.contract_amount)}`);
  }
  if (result.fields.invoiced_amount !== undefined) {
    lines.push(`已开票总金额: ${formatFieldAmount(result.fields.invoiced_amount)}`);
  }
  if (result.fields.current_amount !== undefined) {
    lines.push(`本次开票金额: ${formatFieldAmount(result.fields.current_amount)}`);
  }
  if (result.fields.received_amount !== undefined) {
    lines.push(`已收款总金额: ${formatFieldAmount(result.fields.received_amount)}`);
  }
  if (result.derived.unpaid_formatted) lines.push(`已开票未收款: ${result.derived.unpaid_formatted}`);
  if (result.fields.confirmed_amount) {
    lines.push(`累计确权额: ${formatFieldAmount(result.fields.confirmed_amount)}`);
  }
  lines.push('');
...
function generateMarkdownReport(result) {
  ...
  lines.push(`| 项目 | 金额 |`);
  lines.push(`|------|------|`);
  if (result.fields.contract_amount !== undefined) lines.push(`| 合同总金额 | ${result.fields.contract_amount} |`);
  if (result.fields.invoiced_amount !== undefined) lines.push(`| 已开票总金额 | ${result.fields.invoiced_amount} |`);
  if (result.fields.current_amount !== undefined) lines.push(`| 本次开票金额 | ${result.fields.current_amount} |`);
  if (result.fields.received_amount !== undefined) lines.push(`| 已收款总金额 | ${result.fields.received_amount} |`);
  if (result.derived.unpaid_formatted) lines.push(`| 已开票未收款 | ${result.derived.unpaid_formatted} |`);
  lines.push('');
...
module.exports = { generateTextReport, generateJsonReport, generateMarkdownReport };
