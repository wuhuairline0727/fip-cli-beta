...
function buildExtractionCode(config) {
  ...
  // 序列化 tables
  const tablesArray = tables.map((t) => {
    const cols = (t.columns || []).map((c) => {
      const typePart = c.type ? `, type: ${JSON.stringify(c.type)}` : '';
      return `{ header: ${JSON.stringify(c.header)}, field: ${JSON.stringify(c.field)}${typePart} }`;
    }).join(', ');
...
${inputFieldsEntries}
  ...
  for (const tableDef of tables) {
    ...
    for (const colDef of tableDef.columns) {
      ...
      if (idx !== -1) {
        colIndexMap[colDef.field] = { index: idx, type: colDef.type || 'string' };
      }
    }
    const dataRows = [];
