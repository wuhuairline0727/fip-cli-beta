...
function postProcessYjk(data) {
  ...
  if (Array.isArray(surchargeTable) && surchargeTable.length > 0) {
    result.surcharge_prepayment = surchargeTable.map((row) => {
      ...
      const localEduSurcharge = safeParseAmount(rowCopy.local_education_surcharge);
      const totalSurcharge = safeParseAmount(rowCopy.total_surcharge);

      if (urbanTax && urbanTax > 0) {
        rowCopy.urban_maintenance_tax_rate = ((urbanTax / vatPrepayment6) * 100).toFixed(2) + '%';
      }
      if (eduSurcharge && eduSurcharge > 0) {
        rowCopy.education_surcharge_rate = ((eduSurcharge / vatPrepayment6) * 100).toFixed(2) + '%';
      }
      if (localEduSurcharge && localEduSurcharge > 0) {
        rowCopy.local_education_surcharge_rate = ((localEduSurcharge / vatPrepayment6) * 100).toFixed(2) + '%';
      }
      if (totalSurcharge && totalSurcharge > 0) {
        rowCopy.total_surcharge_rate = ((totalSurcharge / vatPrepayment6) * 100).toFixed(2) + '%';
      }
    ...
    result.surcharge_prepayment.forEach((row) => {
      if (row.urban_maintenance_tax_rate) {
        result.surcharge_tax_rates['城市维护建设税'] = row.urban_maintenance_tax_rate;
      }
      if (row.education_surcharge_rate) {
        result.surcharge_tax_rates['教育费及附加'] = row.education_surcharge_rate;
      }
      if (row.local_education_surcharge_rate) {
        result.surcharge_tax_rates['地方教育费及附加'] = row.local_education_surcharge_rate;
      }
  ...
  result.vat_summary = {
    ...
    subcontract_invoice_deduction_2: data.subcontract_invoice_deduction_2 || null,
