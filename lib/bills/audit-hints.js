...
function generateAuditHints(data, billType) {
  ...
  const allocation = Array.isArray(data.expense_allocation) && data.expense_allocation.length > 0
    ? data.expense_allocation[0]
    : null;

  // 规则 a: budget_expense_match
  // 检查预算项目与报销费用事项是否一致
  if (allocation && allocation.budget_item !== undefined && allocation.expense_item !== undefined) {
    ...
    const isMatch = budgetKeywords.length > 0 && expenseKeywords.length > 0 &&
  ...
  if (allocation && allocation.expense_item !== undefined && reason) {
    ...
    const isContained = keywords.length > 0 && keywords.some(kw => reasonText.includes(kw));
  ...
  // 检查预算余额是否充足
  const amount = allocation && allocation.amount !== undefined ? allocation.amount : data.amount;
  if (allocation && allocation.budget_balance !== undefined && amount !== undefined) {
    const budgetBalance = Number(allocation.budget_balance);
    const amountNum = Number(amount);
    const isSufficient = !Number.isNaN(budgetBalance) && !Number.isNaN(amountNum) && budgetBalance >= amountNum;
