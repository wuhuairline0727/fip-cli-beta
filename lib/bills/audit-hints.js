...
function generateAuditHints(data, billType) {
  ...
  // 检查预算余额是否充足
  if (allocation && allocation.budget_balance !== undefined && data.amount !== undefined) {
    const budgetBalance = Number(allocation.budget_balance);
    const amount = Number(data.amount);
    const isSufficient = !Number.isNaN(budgetBalance) && !Number.isNaN(amount) && budgetBalance >= amount;
    ...
    hints.push({
      ...
      message: isSufficient
        ? '预算余额充足'
        : `预算余额 ${budgetBalance} 不足，需要 ${amount}`,
    });
  }
