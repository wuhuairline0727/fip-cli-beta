...
async function closeSwitchOrgDialog(action = 'cancel') {
  ...
  if (btnResult?.found) {
    ...
    debug('closeSwitchOrgDialog: clicked', targetText, 'at', btnResult.x, btnResult.y);
    return true;
  }
...
async function queryAndSelectInPopup(fieldLabel, queryText, inputId) {
  ...
  if (!btnResult?.found) {
    throw new Error(`未找到 ${fieldLabel} 的按钮: ${btnResult?.reason || 'unknown'}`);
  }
  ...
  if (!fillResult.data?.value?.found) {
    throw new Error(`弹窗查询框填充失败: ${fillResult.data?.value?.reason || 'unknown'}`);
  }
...
async function clickSystemConfirm() {
  ...
  while (attempts < 10 && !confirmBtn) {
    ...
    debug('clickSystemConfirm: attempt', attempts, 'result=', JSON.stringify(result));
...
async function selectFirstDepartment() {
  ...
  // 1. 打开部门弹窗
  const btnResult = await cdpUtils.cdpFindPickerButtonByInputId('DataSetFieldComboBox2-input');
  if (!btnResult?.found) {
...
async function tryAutoSelect(fields, options) {
  ...
  try {
    ...
    if (options.organization) {
      await queryAndSelectInPopup('组织机构', options.organization, 'DataSetFieldComboBox1-input');
      result.organization = { selected: true, name: options.organization };
    }

    // 2. 选择项目
    if (options.project) {
      await queryAndSelectInPopup('项目名称', options.project, 'DataSetFieldComboBox3-input');
    ...
    if (options.department) {
      await queryAndSelectInPopup('部门名称', options.department, 'DataSetFieldComboBox2-input');
      result.department = { selected: true, name: options.department };
    } else {
      // 如果没有指定部门，尝试选择第一个可用部门
      debug('tryAutoSelect: no department specified, attempting to select first available');
      try {
