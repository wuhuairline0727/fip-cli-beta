...
async function closeBill() {
  ...
  const isHomePage = (url.includes('#/dashboard') || url.includes('CSCPortal.jsp')) && !url.includes('FLOW_');
  if (isHomePage) {
    console.log('已返回首页');
  ...
  const isHomePage2 = (url2.includes('#/dashboard') || url2.includes('CSCPortal.jsp')) && !url2.includes('FLOW_');
  if (isHomePage2) {
    console.log('已返回首页');
