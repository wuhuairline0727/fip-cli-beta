...
function extractBillInfo(bill) {
  ...
  if (!amount && details.length > 0) {
    amount = details.reduce((sum, d) => sum + (parseFloat(d.amount_with_tax) || parseFloat(d.amount) || 0), 0);
  }

  // 事由
  let reason = bill.payment_reason || bill.reimbursement_reason;
  if (!reason && details.length > 0 && details[0].reason_summary) reason = details[0].reason_summary;
  if (!reason && details.length > 0 && details[0].reason) reason = details[0].reason;
  ...
  // 会计科目
  let accountSubject = '—';
  const subjects = [...new Set(details.map(d => d.expense_item).filter(Boolean))];
  ...
  return {
    ...
    reason: reason || '—',
    accountSubject,
    budgetProject: alloc ? (alloc.budget_item || alloc.budget_project || '—') : '—',
...
function generateReport(bills, title = '单据合并报表') {
  const lines = [];
  const total = bills.reduce((sum, info) => sum + (typeof info.amount === 'number' ? info.amount : 0), 0);

  // 表头
  lines.push('');
  lines.push('╔══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╗');
  lines.push('║' + title.padStart(Math.floor((166 + title.length) / 2)).padEnd(166) + '║');
  lines.push('╠══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╣');
  lines.push('║ 序号 │ 单据类型      │ 单据编号           │ 提单人  │ 部门      │ 金额(元)  │ 会计科目      │ 预算项目    │ 预算余额(元)  │ 审核状态            ║');
  lines.push('╠══════╪═══════════════╪════════════════════╪═════════╪═══════════╪═══════════╪═══════════════╪═════════════╪═══════════════╪═════════════════════╣');
  ...
  // 合计行
  lines.push('╠══════╧═══════════════╧════════════════════╧═════════╧═══════════╧═══════════╧═══════════════╧═════════════╧═══════════════╧═════════════════════╣');
  lines.push('║ 合计金额：¥' + formatMoney(total) + ' '.repeat(154 - formatMoney(total).length) + '║');
  lines.push('╚══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╝');
