...
| 单据代码 | 单据名称 | 配置文件 | 功能 |
|----------|----------|----------|------|
| SLBX | 境内差旅报销单 | `bills/config/domestic-travel.js` | 费用明细、预算分配、交通费/补助费 |
| TBX | 通用报销单 | `bills/config/general-expense.js` | 费用明细、预算分配 |
| CFK | 对外成本费用付款申请 | `bills/config/external-payment.js` | 费用明细、支付信息 |
| CBX | 差旅费报销 | `bills/config/travel-expense.js` | 差旅明细、补助信息 |
...
| 单据代码 | 单据名称 | 配置/模块 | 功能 |
|----------|----------|-----------|------|
| YJK | 预缴计算单 | `bills/config/yjk.js` | 增值税/所得税/附加税费预缴 |
| KP | 建筑施工开票单 | `audit/` 目录 | 合同金额核对、累计确权额核对、附件完整性 |

**税务台账**:
| 台账名称 | 模块文件 | CLI 命令 |
|----------|----------|----------|
| 未开票收入台账 | `ledgers/unbilled-income.js` | `export-unbilled` |
| 进项转出明细台账 | `ledgers/input-transfer.js` | `export-input-transfer` |
| 销项发票明细台账 | `ledgers/output-invoice.js` | `export-output-invoice` |
| 增值税预缴款台账 | `ledgers/vat-prepayment.js` | `export-vat-prepayment` |
| 旅客运输服务台账 | `ledgers/passenger-transport.js` | `export-passenger-transport` |
...
实现位置: `lib/bills/extractor.js` → `postProcessYjk()`

## 弹窗检测策略

`lib/utils/dialog.js` 采用多策略检测：
